...
TOPICS.class_attr = {
  ...
  code: [
    ...
    { c: '속성값을 이용해 계산한 결과를 출력하세요.', i: 'class Rectangle:\n    pass\nr = Rectangle()\nr.width = 4\nr.height = 5\nprint(r.width + r.height)  # 곱셈으로 고치세요!', e: '20', h: '*를 사용하세요: print(r.width * r.height)' },
    { c: '객체 속성을 이용해 자기소개 문장을 만들어 출력하세요.', i: "class Person:\n    pass\np = Person()\np.name = '에그'\np.age = 5\nprint(p.name + '는 ' + p.age + '살입니다')  # 에러가 나요!", e: '에그는 5살입니다', h: 'str(p.age)로 문자열로 바꿔주세요.' },
    { c: '메서드 안에서 속성을 설정하는 방식으로 바꿔보세요.', i: "class Dog:\n    def set_name(self, name):\n        name = name  # self가 빠졌어요!\nd = Dog()\nd.set_name('초코')\nprint(d.name)", e: '초코', h: 'self.name = name 으로 고치세요.' },
    { c: '메서드로 속성값을 증가시키는 코드를 작성하세요.', i: 'class Counter:\n    def set_count(self, n):\n        self.count = n\n    def increase(self):\n        self.count += 1\nc = Counter()\nc.set_count(0)\nc.increase()\nprint(c.count)  # increase()를 한 번 더 호출하세요.', e: '2', h: 'c.increase()를 한 번 더 추가하세요.' },
    { c: '객체 리스트를 만들어 각각 다른 속성값을 부여한 뒤 순회하며 출력하세요.', i: "class Dog:\n    pass\nnames = ['초코', '뭉치']\ndogs = []\nfor n in names:\n    d = Dog()\n    d.name = n\n    dogs.append(d)\n# dogs를 순회하며 각 d.name을 출력하세요.", e: '초코\n뭉치', h: 'for d in dogs:\n    print(d.name)' },
    { c: '메서드로 속성을 설정하고, 다른 메서드로 조건에 따라 다른 값을 반환하세요.', i: "class Student:\n    def set_score(self, score):\n        self.score = score\n    def result(self):\n        if self.score >= 60:\n            return '합격'\n        return '불합격'\ns = Student()\ns.set_score(50)\nprint(s.result())  # 75점으로 바꿔서 합격이 나오게 하세요.", e: '합격', h: 's.set_score(75)로 바꾸세요.' },
...
TOPICS.class_method = {
  ...
  code: [
    ...
    { c: '메서드로 여러 번 값을 누적해보세요.', i: 'class Wallet:\n    def set_gold(self, g):\n        self.gold = g\n    def earn(self, amount):\n        self.gold += amount\nw = Wallet()\nw.set_gold(0)\nw.earn(50)\nprint(w.gold)  # earn(30)을 한 번 더 호출하세요.', e: '80', h: 'w.earn(30)을 추가하세요.' },
    { c: '메서드로 속성값을 조회해 조건에 따라 다른 결과를 반환하세요.', i: 'class Character:\n    def set_hp(self, hp):\n        self.hp = hp\n    def is_alive(self):\n        return self.hp < 0  # 부등호가 반대예요!\nc = Character()\nc.set_hp(0)\nprint(c.is_alive())', e: 'False', h: 'return self.hp > 0 으로 고치세요.' },
    { c: '두 메서드가 같은 속성을 공유하는지 확인하세요.', i: 'class Counter:\n    def set_count(self, n):\n        self.count = n\n    def show(self):\n        print(self.cnt)  # 속성명이 틀렸어요!\nc = Counter()\nc.set_count(5)\nc.show()', e: '5', h: 'self.count로 고치세요.' },
    { c: '메서드로 속성을 여러 번 조작한 뒤 결과를 확인하세요.', i: 'class Character:\n    def set_hp(self, hp):\n        self.hp = hp\n    def damage(self, n):\n        self.hp -= n\n    def heal(self, n):\n        self.hp += n\nc = Character()\nc.set_hp(100)\nc.damage(30)\nprint(c.hp)  # heal(10)을 추가로 호출하세요.', e: '80', h: 'c.heal(10)을 damage 다음에 추가하세요.' },
    { c: '객체 두 개가 각자의 상태를 독립적으로 유지하는지 확인하세요.', i: 'class Character:\n    def set_hp(self, hp):\n        self.hp = hp\n    def damage(self, n):\n        self.hp -= n\na = Character()\na.set_hp(100)\nb = Character()\nb.set_hp(100)\na.damage(50)\nb.damage(50)\nprint(a.hp, b.hp)  # b는 데미지를 입으면 안 돼요!', e: '50 100', h: 'b.damage(50) 줄을 지우세요.' },
    { c: '메서드가 조건에 따라 다른 메서드를 호출하도록 만들어보세요.', i: "class Character:\n    def set_hp(self, hp):\n        self.hp = hp\n    def damage(self, n):\n        self.hp -= n\n        if self.hp <= 0:\n            pass  # die()를 호출해야 해요!\n    def die(self):\n        print('사망')\nc = Character()\nc.set_hp(10)\nc.damage(20)", e: '사망', h: 'pass 대신 self.die()를 호출하세요.' },
    { c: '순서대로 여러 상태 변경 메서드를 호출해 최종 결과를 확인하세요.', i: 'class Inventory:\n    def set_items(self, n):\n        self.items = n\n    def add(self, n):\n        self.items += n\n    def remove(self, n):\n        self.items -= n\ninv = Inventory()\ninv.set_items(10)\ninv.add(5)\nprint(inv.items)  # remove(3)을 추가로 호출하세요.', e: '12', h: 'inv.remove(3)을 add(5) 다음에 추가하세요.' },
    { c: '동일 클래스의 여러 객체를 리스트로 관리하며 각자 상태를 변경해보세요.', i: 'class Character:\n    def set_hp(self, hp):\n        self.hp = hp\n    def damage(self, n):\n        self.hp -= n\nchars = [Character(), Character()]\nfor c in chars:\n    c.set_hp(100)\nchars[1].damage(30)  # 인덱스가 틀렸어요!\nprint(chars[0].hp, chars[1].hp)', e: '70 100', h: 'chars[0].damage(30)으로 고치세요.' },
    { c: '경험치가 100 이상이면 레벨을 올리고 경험치를 0으로 초기화하는 메서드를 만드세요.', i: 'class Player:\n    def set_stat(self, level, exp):\n        self.level = level\n        self.exp = exp\n    def gain_exp(self, n):\n        self.exp += n\n        if self.exp >= 100:\n            self.level += 1\n            # exp를 0으로 초기화하는 줄이 빠졌어요!\np = Player()\np.set_stat(1, 90)\np.gain_exp(20)\nprint(p.level, p.exp)', e: '2 10', h: 'self.exp = 0을 if문 안에 추가하세요.' },
