...
TOPICS.dict_basic = {
  ...
  code: [
    ...
    { c: '빈 딕셔너리를 만들고 그대로 출력하세요.', i: 'cart = {}\nprint(cart)', e: '{}', h: '이미 정답입니다. 실행해보세요.' },
    ...
    { c: "학생 정보 {'name':'철수','score':90}을 만들고 그대로 출력하세요.", i: "student = {'name': '철수', 'score': 90}\nprint(student)", e: "{'name': '철수', 'score': 90}", h: '이미 정답입니다. 실행해보세요.' },
...
TOPICS.args_star = {
  ...
  code: [
    ...
    { c: '*args로 받은 숫자들의 최댓값을 출력하는 함수를 만들어 3,7,2로 호출하세요.', i: 'def biggest(*nums):\n    print(max(nums))\nbiggest(3, 7, 2)', e: '7', h: '이미 정답입니다. 실행해보세요.' },
    { c: '*args를 인자 없이 호출하면 빈 튜플이 되는지 확인하세요.', i: 'def show(*args):\n    print(args)\nshow()', e: '()', h: '이미 정답입니다. 실행해보세요.' },
    { c: '*args로 받은 문자열들을 공백으로 이어붙여 출력하세요.', i: "def join_words(*words):\n    print(' '.join(words))\njoin_words('나는', '코딩을', '좋아해')", e: '나는 코딩을 좋아해', h: '이미 정답입니다. 실행해보세요.' },
    { c: '고정 매개변수(title)와 *args를 함께 받는 함수를 만들어 호출하세요.', i: "def show(title, *items):\n    print(title, items)\nshow('장바구니', '사과', '바나나')", e: "장바구니 ('사과', '바나나')", h: '이미 정답입니다. 실행해보세요.' },
    { c: '*args로 받은 숫자들의 개수를 len()으로 출력하세요.', i: 'def count(*nums):\n    print(len(nums))\ncount(1, 2, 3, 4, 5)', e: '5', h: '이미 정답입니다. 실행해보세요.' },
...
TOPICS.lambda = {
  ...
  code: [
    ...
    { c: '두 수 중 큰 값을 반환하는 lambda 함수를 만들어 7, 3으로 호출하세요.', i: 'bigger = lambda a, b: a if a > b else b\nprint(bigger(7, 3))', e: '7', h: '이미 정답입니다. 실행해보세요.' },
    { c: "이름을 받아 인사말을 반환하는 lambda 함수를 만들어 '에그'로 호출하세요.", i: "greet = lambda name: '안녕, ' + name\nprint(greet('에그'))", e: '안녕, 에그', h: '이미 정답입니다. 실행해보세요.' },
    { c: '숫자가 짝수인지 확인하는 lambda 함수를 만들어 4로 호출하세요.', i: 'is_even = lambda n: n % 2 == 0\nprint(is_even(4))', e: 'True', h: '이미 정답입니다. 실행해보세요.' },
    { c: '세 수를 더하는 lambda 함수를 만들어 1,2,3으로 호출하세요.', i: '# 세 매개변수를 받는 lambda를 만드세요.', e: '6', h: 'add3 = lambda a, b, c: a + b + c\nprint(add3(1, 2, 3))' },
    { c: '리스트를 정렬할 때 key로 lambda를 사용해보세요 (문자열 길이 기준).', i: "words = ['banana', 'kiwi', 'fig']\nwords.sort(key=lambda w: len(w))\nprint(words)", e: "['fig', 'kiwi', 'banana']", h: '이미 정답입니다. 실행해보세요.' },
    { c: '딕셔너리 리스트를 나이 기준으로 정렬해보세요.', i: "people = [{'name': '철수', 'age': 30}, {'name': '영희', 'age': 20}]\npeople.sort(key=lambda p: p['age'])\nprint(people[0]['name'])", e: '영희', h: '이미 정답입니다. 실행해보세요.' },
    { c: 'lambda로 만든 함수를 변수 없이 즉시 호출해보세요.', i: 'print((lambda x: x * 2)(5))', e: '10', h: '이미 정답입니다. 실행해보세요.' },
    { c: '두 수의 나머지를 구하는 lambda를 만들어 17, 5로 호출하세요.', i: '# lambda를 만드세요.', e: '2', h: 'mod = lambda a, b: a % b\nprint(mod(17, 5))' },
    { c: '숫자 리스트를 lambda로 내림차순 정렬해보세요.', i: 'nums = [3, 1, 4, 1, 5]\nnums.sort(key=lambda x: -x)\nprint(nums)', e: '[5, 4, 3, 1, 1]', h: '이미 정답입니다. 실행해보세요.' },
...
TOPICS.map_func = {
  ...
  code: [
    ...
    { c: "문자열 리스트 ['1','2','3']을 숫자로 변환해 출력하세요.", i: "strs = ['1', '2', '3']\nprint(list(map(int, strs)))", e: '[1, 2, 3]', h: '이미 정답입니다. 실행해보세요.' },
    { c: '숫자 리스트 [1,2,3]을 문자열로 변환해 출력하세요.', i: 'nums = [1, 2, 3]\nprint(list(map(str, nums)))', e: "['1', '2', '3']", h: '이미 정답입니다. 실행해보세요.' },
    ...
    { c: '두 리스트 [1,2,3]과 [10,20,30]을 더해 [11,22,33]을 만드세요.', i: 'a = [1, 2, 3]\nb = [10, 20, 30]\nprint(list(map(lambda x, y: x + y, a, b)))', e: '[11, 22, 33]', h: '이미 정답입니다. 실행해보세요.' },
...
TOPICS.except_as = {
  ...
  code: [
    ...
    { c: '에러 메시지를 e로 받아 그대로 출력하세요.', i: 'try:\n    x = 1 / 0\nexcept Exception as e:\n    print(e)', e: 'division by zero', h: '이미 정답입니다. 실행해보세요.' },
    { c: 'KeyError의 에러 메시지를 확인하세요.', i: "d = {'a': 1}\ntry:\n    print(d['b'])\nexcept KeyError as e:\n    print(e)", e: "'b'", h: '이미 정답입니다. 실행해보세요.' },
    { c: '일반 Exception으로 에러를 잡아 타입 이름을 출력하세요.', i: "try:\n    print(1 + '1')\nexcept Exception as e:\n    print(type(e).__name__)", e: 'TypeError', h: '이미 정답입니다. 실행해보세요.' },
    { c: "에러 메시지를 활용해 '에러: 메시지' 형태로 출력하세요.", i: "try:\n    print(10 / 0)\nexcept Exception as e:\n    print('에러: ' + str(e))", e: '에러: division by zero', h: '이미 정답입니다. 실행해보세요.' },
    { c: '함수 안에서 예외를 잡아 메시지를 반환하세요.', i: 'def safe_div(a, b):\n    try:\n        return a / b\n    except Exception as e:\n        return str(e)\nprint(safe_div(5, 0))', e: 'division by zero', h: '이미 정답입니다. 실행해보세요.' },
    { c: '여러 나눗셈을 시도하며 에러가 난 경우 메시지를 출력하세요.', i: 'pairs = [(10, 2), (5, 0)]\nfor a, b in pairs:\n    try:\n        print(a / b)\n    except Exception as e:\n        print(str(e))', e: 'division by zero', h: '이미 정답입니다. 마지막 줄만 확인해요.' },
    { c: '에러 타입 이름만 추출해 출력하세요.', i: 'try:\n    print([1, 2][10])\nexcept Exception as e:\n    print(type(e).__name__)', e: 'IndexError', h: '이미 정답입니다. 실행해보세요.' },
    { c: '딕셔너리 접근 에러의 타입 이름을 출력하세요.', i: "d = {}\ntry:\n    print(d['x'])\nexcept Exception as e:\n    print(type(e).__name__)", e: 'KeyError', h: '이미 정답입니다. 실행해보세요.' },
    { c: '함수에서 에러 타입에 따라 다른 메시지를 만들어 반환하세요.', i: "def describe(f):\n    try:\n        f()\n    except Exception as e:\n        return type(e).__name__ + ': ' + str(e)\nprint(describe(lambda: 1 / 0))", e: 'ZeroDivisionError: division by zero', h: '이미 정답입니다. 실행해보세요.' },
...
TOPICS.class_attr = {
  ...
  code: [
    ...
    { c: '객체의 속성값을 나중에 바꿔보세요.', i: 'class Counter:\n    pass\nc = Counter()\nc.count = 0\nc.count = 5\nprint(c.count)', e: '5', h: '이미 정답입니다. 실행해보세요.' },
    { c: '객체에 여러 속성(hp, mp)을 부여해 출력하세요.', i: 'class Character:\n    pass\nchar = Character()\nchar.hp = 100\nchar.mp = 50\nprint(char.hp, char.mp)', e: '100 50', h: '이미 정답입니다. 실행해보세요.' },
