  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={onCancel}>
      <div
        ...
        className={`w-full max-w-sm rounded-3xl p-6 shadow-2xl border-2 ${isDark ? 'bg-[#2b3a42] border-[#3b4a52] text-white' : 'bg-white border-gray-200 text-gray-800'}`}
      >
        ...
        <div className="flex gap-3">
          <button
            onClick={onCancel}
            className={`flex-1 py-3 rounded-xl font-bold transition ${isDark ? 'bg-[#181A20] hover:bg-[#20262b] text-gray-300' : 'bg-gray-100 hover:bg-gray-200 text-gray-600'}`}
          >
            {cancelLabel}
          </button>
          <button
